...
public class FleetResource {
    ...
    public Response getModelRecommendation(@QueryParam("routeId") String routeId,
                                            @QueryParam("occupancy") Integer occupancy) {
        ...
        double targetOccupancy = (occupancy == null ? 80 : occupancy) / 100.0;
        try {
            return Response.ok(recommendBusModelUseCase.execute(routeId, targetOccupancy)).build();
        } catch (RouteNotFoundException | NoDemandDataException e) {
            return Response.status(Response.Status.NOT_FOUND).entity(e.getMessage()).build();
